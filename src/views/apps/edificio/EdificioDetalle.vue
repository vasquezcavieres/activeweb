<template>
  ...
  <div class="card">
    <div class="card-header">
      ...
      <div class="card-title">
        <h2 class="color-payment">{{ edificioDetails.codigoEdificio }} - {{ edificioDetails.nombre }}</h2>
      </div>
      <!--end::Card title-->
    </div>
  </div>
    ...
    <div class="card-body pt-0 mt-2">
      ...
      <div class="row">
        <div class="col-4">
          ...
          <p><stron>{{ edificioDetails.direccion }}</stron></p>
        </div>
        <div class="col-4">
          <p class="color-payment"><strong>Ciudad</strong></p>
          <p>{{ edificioDetails.ciudad }}</p>
        </div>
        <div class="col-4">
          <p class="color-payment"><strong>Comuna</strong></p>
          <p>{{ edificioDetails.comuna }}</p>
        </div>
      </div>
      <!-- <div class="row">
        <div class="col-4">
  ...
  <div class="card mt-5">
    <div class="card-header">
                          <!--begin::Card title-->
                          <div class="card-title">
                            <h2 class="color-payment">Cargas de Saldo</h2>
                          </div>
                          <!--end::Card title-->
                        </div>
    <!--begin::Card body-->
...
export default defineComponent({
  ...
  setup() {
    ...
    onMounted(() => {
      obtenerEdificio(codigoEdificio);
    // obtenerConvenioEdificio(codigoEdificio);
      obtenerPagosEdificio(codigoEdificio);
    });
  ...
  const obtenerEdificio = (id) =>{
      store
        .getEdificio(id)
        .then(() => {
          loading.value = false;
          edificioDetails.value = store.currentEdificio;
        })
        .catch(() => {
  ...
  const obtenerConvenioEdificio = (id) =>{
    storeConvenio
        .getConvenioEdificio(id)
        .then(() => {        
  ...
  const obtenerPagosEdificio = (codigoEdificio) => {
      storePago.getPagosEdificio(codigoEdificio)
        .then(() => {
          loading.value = false;
        })
        .catch(() => {
    ...
    return {
      ...
      allPagos,
      obtenerEdificio,
      obtenerPagosEdificio,
      obtenerConvenioEdificio,
      convenioDetails,
      edificioDetails,           
      selectedEdificios,
      loading,
