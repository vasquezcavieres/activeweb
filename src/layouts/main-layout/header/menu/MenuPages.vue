<template>
    ...
    >
      ...
      >
        <template v-for="(menuItem, j) in item.pages" :key="j">
          ...
          >
            <span
              v-if="menuItem.route && checkPermission(menuItem)"
              class="menu-link py-3"
  ...
  <div
    data-kt-menu-trigger="click"
    data-kt-menu-placement="bottom-start"
    class="d-none menu-item menu-lg-down-accordion me-lg-1"
  >
...
export default defineComponent({
  ...
  setup() {
    ...
    const checkPermission = (menuItem) => {
      if(typeof(menuItem.perminission)=='undefined')
        return true;
      const role = menuItem.perminission as string;
      let rol = authStore.user?.codigoRol;

      if(rol == menuItem.perminission)
        return true;
      else
        return false;
